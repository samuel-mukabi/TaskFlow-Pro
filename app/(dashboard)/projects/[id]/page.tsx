...
import { fetchProjectById, fetchProjectMembers } from "@/app/actions/projects.ts";
import { Project, Tasks, TeamMember } from "@/types";
import { fetchTasks } from "@/app/actions/tasks.ts";
import { calculateProjectProgress, getDate } from "@/app/actions/conversions.ts";
...
const ProjectDetail = ({ params }: PageProps) => {
    ...
    const startDate = getDate(project?.start_date)
    const endDate = getDate(project?.end_date)
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                const team_members = await fetchProjectMembers(projectId);
                if (team_members) {
    ...
    const totalProgress = calculateProjectProgress(tasks)


    return (
        <div className={`p-8  min-h-screen ${addMember && 'overflow-hidden relative'}`}>
            ...
            <div className="mb-6">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex -space-x-2">
                            ...
                            {teamMembers.length > 3 && (
                                <div
                                    className="w-10 h-10 bg-slate-300 rounded-full border-2 border-white flex items-center justify-center text-sm font-semibold text-slate-600">
                                    +{teamMembers.length - 3}
                                </div>
                            )}
                            {teamMembers.length === 0 && (
                                <div
                                    className="w-10 h-10 bg-slate-100 rounded-full border-2 border-white flex items-center justify-center text-xs text-slate-400">
                                    None
                                </div>
                            )}
                        </div>
                        <button
                            ...
                            className="px-3 py-2 text-sm bg-transparent border border-neutral-400 text-neutral-800 rounded-md font-semibold hover:border-stone-900 hover:bg-stone-900 hover:text-white transition flex items-center gap-2 cursor-pointer">
                            + Add Member
                        </button>
                        <button
                            className="p-2 bg-white text-slate-700 rounded-lg hover:bg-slate-50 transition">
                ...
                <div className="flex items-center gap-6 mt-6">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-slate-700">Created on <span
                            className="font-semibold">{startDate}</span>
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
                        <svg className="w-5 h-5 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <span className="text-sm text-slate-700">Due <span
                            className="font-semibold">{endDate}</span>
                        </span>
                    </div>
                            ...
                            {ToDoTasks.length > 0 ? ToDoTasks.map((task) => (
                                <div
                                    ...
                                    <div className="flex items-start justify-between mb-2">
                                        ...
                                        {task.priority && (<span
                                            className="px-2 py-0.5 bg-red-100 text-red-700 text-xs font-semibold rounded">{task.priority}</span>)}
